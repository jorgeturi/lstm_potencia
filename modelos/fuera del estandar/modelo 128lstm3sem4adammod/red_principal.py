...
def entrenar_modelo(Xtrain, ytrain, Xval, yval, path_guardado='modelo_entrenado.h5'):
    ...
    values = [0.008, 0.002, 0.001, 0.0001, 0.00005, 0.00001, 0.000001]  # Learning rates correspondientes a los intervalos
    ...
    #model.add(BatchNormalization())
    #model.add(LSTM(24, return_sequences=False, kernel_initializer=initializer ) )
    #model.add(BatchNormalization())
    ...
    optimizer = Adam(learning_rate=lr_schedule, beta_1=0.85, beta_2=0.68, amsgrad=True)
    ...
    try:
        # Entrenar el modelo con datos de validación, EarlyStopping y ModelCheckpoint
        model.fit(Xtrain, ytrain, epochs=60, verbose=1, batch_size=16,
...
def custom_loss(y_true, y_pred):
    """
    Penaliza fuertemente si la predicción no sigue los cambios bruscos del valor real.
    Reduce la penalización si la predicción capta bien los cambios grandes.
    """
    # Diferencia absoluta entre la predicción y el valor real
    error = K.abs(y_true - y_pred)
    
    # Diferencia entre valores consecutivos en y_true (para detectar cambios bruscos)
    delta_y_true = K.abs(y_true[:, 1:] - y_true[:, :-1])
    
    # Diferencia entre valores consecutivos en y_pred (para ver si el modelo detecta los cambios)
    delta_y_pred = K.abs(y_pred[:, 1:] - y_pred[:, :-1])
    
    # Penalización: Si el modelo predice poca variación cuando el verdadero cambia mucho
    change_penalty = K.abs(delta_y_true - delta_y_pred)
    
    # Ajuste de la penalización (para amplificar errores pequeños y suavizar los grandes)
    alpha = 2.0  # Controla cuánto se amplifican los errores pequeños
    loss = K.mean(error + alpha * change_penalty)
