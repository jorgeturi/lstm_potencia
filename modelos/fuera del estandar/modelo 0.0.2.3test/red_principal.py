...
def custom_huber_loss_with_std(y_true, y_pred, delta=1.0):
    # Huber Loss
    residual = tf.abs(y_true - y_pred)
    condition = tf.less(residual, delta)
    squared_loss = 0.5 * tf.square(residual)
    linear_loss = delta * residual - 0.5 * delta**2
    huber_loss = tf.where(condition, squared_loss, linear_loss)
    
    # Penalización por desviación estándar
    std_penalty = tf.math.reduce_std(y_true - y_pred)
    
    # Combinar Huber Loss con la penalización de desviación estándar
    loss = tf.reduce_mean(huber_loss) + 0.1 * std_penalty
    return loss
...
def entrenar_modelo(Xtrain, ytrain, Xval, yval, path_guardado='modelo_entrenado.h5'):
    ...
    optimizer = Adam(learning_rate=lr_schedule, clipnorm=1)
    model.compile(optimizer=optimizer, loss=custom_huber_loss_with_std)
